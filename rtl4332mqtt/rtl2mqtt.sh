...
#/usr/local/bin/rtl_433 -M newmodel -R $PROTOCOL -f $FREQUENCY -F "mqtt://$MQTT_HOST:1883,user=$MQTT_USER,pass=$MQTT_PASS,devices=$MQTT_TOPIC[/model][/id]"
/usr/local/bin/rtl_433 -M newmodel -R $PROTOCOL -f $FREQUENCY -F "mqtt://$MQTT_HOST:1883,user=$MQTT_USER,pass=$MQTT_PASS,events=$MQTT_TOPIC/[id]"
